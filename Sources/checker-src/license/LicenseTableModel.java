...
public class LicenseTableModel extends AbstractTableModel {
	...
	private String licenseName = "testi";
	private int licenseCount = 0;
	/*Object data[][] = {
			{this.licenseName, new Boolean(false)}
			
	};*/
	//Object[][] data = new Object[50][2];
	Object[][] data;
	Boolean compatible = new Boolean(false);
	String columnNames[] = {loc.lc("License"), "Compatible"};
	
	
	
	public LicenseTableModel(int licenseCount) {
		//this.licenseName = licenseName;
		this.licenseCount = licenseCount;
		data = new Object[licenseCount][2];
		for (int i = 0; i < this.data.length; i++) {
			this.setValueAt("", i, 0);
			this.setValueAt(new Boolean(false), i, 1);
		}
	}
